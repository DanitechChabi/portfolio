 ...
 * (Copie locale du validateur du skill « dataviz » — mêmes seuils, même math.)
 *
 * Usage :
 *   node scripts/validate_palette.js "#a06b1a,#2c4e6e,#2f5d48,#6d4f7c,#c0391b" \
 *     --mode light --surface "#faf7ec"
 *
...
// Node CLI
if (typeof process !== "undefined" && process.argv && process.argv[1] && /validate_palette\.(mjs|js)$/.test(process.argv[1])) {
...
if (typeof document !== "undefined") {
  ...
  if (b?.dataset.palette) {
    ...
    if (!palette.length || badEnum || badHex.length) {
      ...
      const result = ordinal ? validateOrdinal(palette, { mode, surface, pairs }) : validate(palette, { mode, surface, pairs });
